...
from .client_evt_r1.tools.sam3.sam3 import call_sam_service
...
def _prune_messages_for_next_round(
    messages_list,
    used_text_prompts,
    latest_evt_r1.tools.sam3.sam3_text_prompt,
    img_path,
    initial_text_prompt,
):
    ...
    previously_used = (
        [p for p in used_text_prompts if p != latest_evt_r1.tools.sam3.sam3_text_prompt]
        if latest_evt_r1.tools.sam3.sam3_text_prompt
...
def agent_inference(
    ...
    output_dir="../../evt_r1.tools.sam3.sam3_agent_out",
):
    """
    Given a text prompt and an image, this tool will perform all aspects of agentic problem solving,
    while saving evt_r1.tools.sam3.sam3 and MLLM outputs to their respective directories.
    ...
    while generated_text is not None:
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            if current_text_prompt in USED_TEXT_PROMPTS:
                ...
                LATEST_evt_r1.tools.sam3.sam3_TEXT_PROMPT = current_text_prompt
                PATH_TO_LATEST_OUTPUT_JSON = call_sam_service(
                    image_path=img_path,
                    text_prompt=current_text_prompt,
                    output_folder_path=sam_output_dir,
                )
                evt_r1.tools.sam3.sam3_outputs = json.load(open(PATH_TO_LATEST_OUTPUT_JSON, "r"))
                evt_r1.tools.sam3.sam3_output_image_path = evt_r1.tools.sam3.sam3_outputs["output_image_path"]
                num_masks = len(evt_r1.tools.sam3.sam3_outputs["pred_boxes"])
                ...
                if num_masks == 0:
                    print("❌ No masks generated by evt_r1.tools.sam3.sam3, reporting no mask to Qwen.")
                    evt_r1.tools.sam3.sam3_output_text_message = f"The segment_phrase tool did not generate any masks for the text_prompt '{current_text_prompt}'. Now, please call the segment_phrase tool again with a different, perhaps more general, or more creative simple noun phrase text_prompt, while adhering to all the rules stated in the system prompt. Please be reminded that the original user query was '{initial_text_prompt}'."
                    messages.append(
                        {
                            "role": "user",
                            "content": [
                                {"type": "text", "text": evt_r1.tools.sam3.sam3_output_text_message}
                            ],
                        }
                    )
                else:
                    evt_r1.tools.sam3.sam3_output_text_message = rf"The segment_phrase tool generated {num_masks} available masks. All {num_masks} available masks are rendered in this image below, now you must analyze the {num_masks} available mask(s) carefully, compare them against the raw input image and the original user query, and determine your next action. Please be reminded that the original user query was '{initial_text_prompt}'."
                    messages.append(
                        {
                            "role": "user",
                            "content": [
                                {"type": "text", "text": evt_r1.tools.sam3.sam3_output_text_message},
                                {"type": "image", "image": evt_r1.tools.sam3.sam3_output_image_path},
                            ],
                        }
                    )
                print("\n\n>>> evt_r1.tools.sam3.sam3_output_text_message:\n", evt_r1.tools.sam3.sam3_output_text_message)

        elif tool_call["name"] == "examine_each_mask":
            print("🔍 Calling examine_each_mask tool...")
            assert LATEST_evt_r1.tools.sam3.sam3_TEXT_PROMPT != ""
            ...
            for i in range(num_masks):
                ...
                image_w_zoomed_in_mask_i_path = os.path.join(
                    sam_output_dir, rf"{LATEST_evt_r1.tools.sam3.sam3_TEXT_PROMPT}.png".replace("/", "_")
            ...
            image_w_check_masks_path = os.path.join(
                sam_output_dir, rf"{LATEST_evt_r1.tools.sam3.sam3_TEXT_PROMPT}.png"
            ).replace(
                ".png",
