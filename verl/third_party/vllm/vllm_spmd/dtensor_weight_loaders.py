...
def generate_qwen_parallelize_plan() -> Dict[str, Any]:
    col_parallel = [Shard(0)]
    row_parallel = [Shard(1)]
    replicated   = [Replicate()]

    # Core base mappings
    plan = {
        # --- Embedding & Final Norm ---
        # Added variants without '.weight' to catch parameter name edge cases
        "visual.patch_embed.proj":                  replicated,
        "visual.patch_embed.proj.weight":           replicated,
        "visual.patch_embed.proj.bias":             replicated,
        "visual.pos_embed":                         replicated,
        
        # Vision blocks (Qwen3-VL uses standard transformer blocks for vision)
        "visual.blocks.{i}.norm1":                  replicated,
        "visual.blocks.{i}.norm1.weight":           replicated,
        "visual.blocks.{i}.norm1.bias":           replicated,
        "visual.blocks.{i}.norm2":                  replicated,
        "visual.blocks.{i}.norm2.weight":           replicated,
        "visual.blocks.{i}.norm2.bias":           replicated,
        
        # Vision Attention (Typically replicated unless you run vision-specific TP)
        "visual.blocks.{i}.attn.qkv":               replicated,
        "visual.blocks.{i}.attn.qkv.weight":        replicated,
        "visual.blocks.{i}.attn.qkv.bias":        replicated,
        "visual.blocks.{i}.attn.proj":              replicated,
        "visual.blocks.{i}.attn.proj.weight":       replicated,
        "visual.blocks.{i}.attn.proj.bias":        replicated,
        
        # Vision MLP Block
        "visual.blocks.{i}.mlp.linear_fc1":                replicated,
        "visual.blocks.{i}.mlp.linear_fc1.weight":         replicated,
        "visual.blocks.{i}.mlp.linear_fc1.bias":         replicated,
        "visual.blocks.{i}.mlp.linear_fc2":                replicated,
        "visual.blocks.{i}.mlp.linear_fc2.weight":         replicated,
        "visual.blocks.{i}.mlp.linear_fc2.bias":         replicated,
        
        # Vision Output / Merging / RoPE components
        "visual.merger.norm":                       replicated,
        "visual.merger.norm.weight":                       replicated,
        "visual.merger.norm.bias":                       replicated,
        "visual.merger.linear_fc1":                replicated,
        "visual.merger.linear_fc1.weight":                replicated,
        "visual.merger.linear_fc1.bias":                replicated,
        "visual.merger.linear_fc2":                replicated,
        "visual.merger.linear_fc2.weight":                replicated,
        "visual.merger.linear_fc2.bias":                replicated,

        "visual.deepstack_merger_list.{i}.norm": replicated,
        "visual.deepstack_merger_list.{i}.norm.weight": replicated,
        "visual.deepstack_merger_list.{i}.norm.bias": replicated,
        "visual.deepstack_merger_list.{i}.linear_fc1": replicated,
        "visual.deepstack_merger_list.{i}.linear_fc1.weight": replicated,
        "visual.deepstack_merger_list.{i}.linear_fc1.bias": replicated,
        "visual.deepstack_merger_list.{i}.linear_fc2": replicated,
        "visual.deepstack_merger_list.{i}.linear_fc2.weight": replicated,
        "visual.deepstack_merger_list.{i}.linear_fc2.bias": replicated,

        "language_model.model.embed_tokens":        replicated,
        ...
        "{i}.self_attn.o_proj":   replicated,
        
        # --- MLP Block ---
        "{i}.mlp.gate_up_proj":   replicated,
        "{i}.mlp.down_proj":      replicated,
        
        # --- Layer Norms ---
        "{i}.input_layernorm":                  replicated,
        "{i}.post_attention_layernorm":         replicated,
        "{i}.self_attn.q_norm":               replicated,
        "{i}.self_attn.k_norm":               replicated,
        "{i}.self_attn.v_norm":               replicated,

    }
    
    # Dynamically unroll for your 128 layers
    unrolled_plan = {}
    for key, placement in plan.items():
        if "{i}" in key:
            for i in range(128):
                unrolled_plan[key.format(i=i)] = placement
        else:
            unrolled_plan[key] = placement
            
    return unrolled_plan

def qwen3vl_dtensor_weight_loader(actor_weights: Dict, vllm_model: nn.Module) -> nn.Module:
    stacked_params_mapping = [
        # (param_name, shard_name, shard_id)
        ("qkv_proj", "q_proj", "q"),
        ("qkv_proj", "k_proj", "k"),
    ...
    parallelize_plan = generate_qwen_parallelize_plan()
    params_dict = dict(vllm_model.named_parameters())
    loaded_params = set()
    for name, loaded_weight in actor_weights.items():
        if "model.visual" in name:
        ...
        for (param_name, shard_name, shard_id) in stacked_params_mapping:
            ...
            if name.endswith(".bias") and name not in params_dict:
                continue
            local_loaded_weight = redistribute_dtensor(param_name=name, loaded_weights=loaded_weight, parallelize_plan=parallelize_plan)
            param = params_dict[name]
            weight_loader = param.weight_loader
            weight_loader(param, local_loaded_weight.to(dtype=param.dtype), shard_id)
            break
...
def redistribute_dtensor(param_name: str, loaded_weights: DTensor, parallelize_plan: Dict = None):
    ...
    if parallelize_plan is not None:
        ...
        # print("No parallelize_plan")
        if isinstance(loaded_weights, DTensor):
            local_loaded_weights = loaded_weights.to_local()
            # print("To local tensor")
        else:
